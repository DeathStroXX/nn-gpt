        ...
        if random.random() < self.mutation_rate:
            if len(possible_values) > 1:
                ...
                for val in current_value:
                    new_val = self.mutate_gene(gene_name, val, possible_values)
                    new_value.append(new_val)
                return new_value
            elif isinstance(current_value, dict):
                mutated = {}
                for key, value in current_value.items():
                    mutated[key] = self.mutate_gene(gene_name, value, possible_values)
                return mutated
            else:
                mutated = self._coerce_gene_value(gene_name, possible_values[0])
                if current_value != mutated:
                    return mutated
